...
useEffect(() => {
  ...
  const unsubscribe = onSnapshot(q, (snapshot) => {
    ...
    // 1. Filtrar las que siguen realmente activas
    const active = allDocs.filter(s => s.status !== 'completada');
    setActiveSessions(active);
    
    // 2. Filtrar las que están completadas (tus 11 usuarios perdidos)
    const completed = allDocs.filter(s => s.status === 'completada');
  ...
  if (sessionId && started && !finished) {
    ...
    // MODIFICACIÓN: Ahora incluimos el objeto 'answers'
    updateDoc(sessionRef, {
      currentQuestion: currentQ + 1,
      timeLeft: timeLeft,
      lastActivity: new Date().toISOString(),
      answers: answers // <-- Esta es la clave para que no aparezcan en 0
    }).catch(err => console.error("Error al sincronizar respuestas:", err));
  }
  // Mantenemos currentQ y answers como dependencias para que se guarde al responder
}, [currentQ, sessionId, started, finished, answers]);
...
<div className="mt-12 bg-card border-amber-500 shadow-lg">
  <h2 className="text-3xl font-bold mb-6 text-amber-900">
    📂 Sesiones Finalizadas (Recuperación)
  </h2>
  <p className="mb-4 text-amber-800 italic">
    Aquí aparecen los usuarios que fueron cerrados forzosamente. Puedes ver sus respuestas parciales.
  </p>
    ...
    <table className="min-w-full">
      <thead>
        <tr className="bg-amber-100">
          ...
          <th className="p-4 text-center">Respuestas enviadas</th>
          <th className="p-4 text-center">Acción</th>
        </tr>
      </thead>
      <tbody>
        {completedSessions.map((session) => (
          <tr key={session.id} className="border-b border-amber-200">
            <td className="p-4 font-bold">{session.name}</td>
            <td className="p-4">{session.idNumber}</td>
            <td className="p-4">{session.selectedAsignatura || session.asignatura}</td>
            <td className="p-4 text-center">
              {Object.keys(session.answers || {}).length} / 45
            </td>
            <td className="p-4 text-center">
              <button
  onClick={() => {
    // Calculamos el puntaje antes de abrir el modal
    const score = calculateScore(session.answers || {});
    setViewingSession({ ...session, calculatedScore: score });
  }}
        ...
        {completedSessions.length === 0 && (
          <tr>
            <td colSpan={5} className="p-8 text-center text-gray-500">
              No hay sesiones finalizadas para recuperar.
            </td>
          </tr>
        )}
      </tbody>
...
{viewingSession && (
  <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
    <div className="bg-white dark:bg-[#333333] w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden border-4 border-indigo-500">
      
      {/* Cabecera */}
      <div className="bg-indigo-600 p-6 text-white flex justify-between items-center">
        <div>
          <h3 className="text-2xl font-bold">{viewingSession.name}</h3>
          <p className="opacity-90">{viewingSession.selectedAsignatura || viewingSession.asignatura}</p>
        </div>
        <button 
      ...
      <div className="p-8 max-h-[70vh] overflow-y-auto">
        <div className="grid grid-cols-2 gap-6 mb-8">
          <div className="p-4 bg-indigo-50 dark:bg-[#444444] rounded-2xl text-center border-2 border-indigo-100">
            <p className="text-sm text-indigo-600 dark:text-indigo-300 font-bold uppercase">Puntaje</p>
            <p className="text-4xl font-black text-indigo-900 dark:text-white">
              {viewingSession.calculatedScore.total.toFixed(1)}
            </p>
          </div>
          <div className={`p-4 rounded-2xl text-center border-2 ${
            viewingSession.calculatedScore.total >= 3.0 
              ? 'bg-emerald-50 border-emerald-200 text-emerald-700' 
              : 'bg-rose-50 border-rose-200 text-rose-700'
          }`}>
            <p className="text-sm font-bold uppercase">Estado</p>
            <p className="text-2xl font-black">
              {viewingSession.calculatedScore.total >= 3.0 ? 'APROBADO' : 'REPROBADO'}
            </p>
          </div>
        </div>

        <h4 className="font-bold mb-4 border-b pb-2">Desglose de respuestas:</h4>
        <div className="space-y-3">
          {viewingSession.calculatedScore.details.map((det: any, idx: number) => (
            <div key={idx} className="flex justify-between items-center p-3 bg-gray-50 dark:bg-[#2d2d2d] rounded-xl text-sm">
              <span className="flex-1 pr-4 text-gray-700 dark:text-gray-300">
                Pregunta {idx + 1}: {det.questionText.substring(0, 60)}...
              </span>
              <span className={`font-bold ${det.isCorrect ? 'text-emerald-600' : 'text-rose-600'}`}>
                {det.isCorrect ? '✓ Correcta' : '✗ Incorrecta'}
              </span>
            </div>
          ))}
        </div>
      ...
      {/* Footer con opción de cerrar definitivo */}
      <div className="p-6 bg-gray-50 dark:bg-[#2d2d2d] border-t flex justify-end gap-4">
        <button 
          onClick={() => setViewingSession(null)}
